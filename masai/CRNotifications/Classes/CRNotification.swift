...
class CRNotification: UIView {
	...
	func showNotification() {
		UIView.animate(withDuration: 0.2, delay: 0.0, options: UIViewAnimationOptions(), animations: {
            var originY  = UIApplication.shared.statusBarFrame.height
            
            if let navBarHeight = UIApplication.shared.keyWindow?.visibleViewController?.navigationController?.navigationBar.frame.size.height {
                originY += navBarHeight
            }
            
			self.frame.origin.y = originY + (self.frame.height * 0.1) + 5
			}, completion: {
				(complete: Bool) in
